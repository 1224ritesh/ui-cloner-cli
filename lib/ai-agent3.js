...
export class AIAgent3 {
  ...
  async cleanWithAI(rawHtml) {
    const prompt = `
You are an expert web developer AI. Your task is to rewrite a website's HTML so it works fully offline.
Follow these steps carefully using Chain of Thought reasoning:

1. Identify all linked CSS, JS, and image files.
2. Identify all inline <style> and <script> blocks.
3. Identify fonts (Google Fonts, local, or external) and make them offline-compatible.
4. Download or inline all small assets (like SVGs, icons) if possible.
5. Combine all CSS (linked + inline) into one file: style.css.
6. Combine all JS (linked + inline) into one file: script.js.
7. Rewrite all HTML references to point to the local files:
   - All CSS links → style.css
   - All JS links → script.js
   - All images → point to local filenames
   - All fonts → use local or embedded fonts
8. Preserve all HTML structure, functionality, and responsiveness.
9. Remove unnecessary external references that are not essential for the site to work offline.
10. Optimize the HTML for clarity and performance (minify inline code if possible, but keep readability).

Return **ONLY** the cleaned, fully offline-ready HTML code. Do not include explanations or comments.
...
`;
    ...
    try {
      console.log(`🤖 Cleaning HTML with AI...`);
      const result = await this.makeAPICall(prompt);
      return this.cleanHTMLResponse(result);
    } catch (error) {
      console.warn(`⚠️ AI cleaning failed: ${error.message}`);
      return rawHtml; // Return original HTML if AI fails
    }
  }

  cleanHTMLResponse(response) {
    // Clean up the AI response to extract just the HTML
    ...
    const htmlStart = html.search(/<\s*!DOCTYPE\s+html/i);
    if (htmlStart > 0) {
      html = html.substring(htmlStart);
    } else {
      const htmlTagStart = html.search(/<\s*html/i);
      if (htmlTagStart > 0) {
  ...
  async processForOffline(html, options = {}) {
    const prompt = `You are an expert web developer AI. Your task is to rewrite a website's HTML so it works fully offline.
Follow these steps carefully using Chain of Thought reasoning:
...
3. Identify fonts (Google Fonts, local, or external) and make them offline-compatible.
4. Download or inline all small assets (like SVGs, icons) if possible.
5. Combine all CSS (linked + inline) into one <style> block in <head>.
6. Combine all JS (linked + inline) into one <script> block before </body>.
7. Rewrite all HTML references to point to the local files:
   - All CSS links → single <style> block with combined CSS
   - All JS links → single <script> block with combined JS
   - All images → point to local filenames (preserve src attributes for assets)
   - All fonts → use local or embedded fonts
8. Preserve all HTML structure, functionality, and responsiveness.
9. Remove unnecessary external references that are not essential for the site to work offline.
10. Optimize the HTML for clarity and performance (minify inline code if possible, but keep readability).
...
${html}`;
    ...
    try {
      console.log(`🔄 Creating offline-ready HTML...`);
      const result = await this.makeAPICall(prompt);
      return result;
    } catch (error) {
      console.error(`❌ Offline processing failed:`, error.message);
      throw new Error(`Offline processing failed: ${error.message}`);
    }
  }

  async processHTML(html, url, options = {}) {
    // If offline processing is requested, use the dedicated offline method
    if (options.offline === true) {
      return await this.processForOffline(html, options);
    }
...
Generate a complete HTML document with embedded CSS that looks like the VS Code homepage.`;
    ...
    // General website cloning approach for other sites
    const prompt = `You are an expert web developer creating a website replica based on this analysis:
...
Create a modern, responsive webpage that captures the essence of the original site.
Include proper navigation, hero section, content areas, and footer.
Use clean HTML5 structure with embedded CSS.
Maintain the visual style and branding of the original.
